...
# Batch processing function for cosine similarity
def batch_cosine_similarity(x_test, X_train, batch_size=100):
    similarities = []
    for i in range(0, len(X_train), batch_size):
        X_train_batch = X_train[i:i + batch_size]
        cos_sim_batch = cosine_similarity(x_test, X_train_batch)
        similarities.append(cos_sim_batch)
    return np.concatenate(similarities, axis=1)
...
def recognize():
    ...
    if file:
        ...
        # Batch processing for cosine similarity to avoid memory issues
        cos_similarities = batch_cosine_similarity(x_test, X_train, batch_size=100)
        ...
        for i in similar_items:
            food_name = list_images[i].split('data\\')[1].split('\\')[0].replace("_", " ")
